<?php 
	...
	$img_economia = $_POST['imgEconomia'];
	 ...
	 $consulta = mysqli_query($cn, "SELECT * FROM `estado` WHERE `id_pais` = '$pais' AND `nome` = '$nome'");
	...
	if (count($exibe) >= 1) {
		...
		$sql = "INSERT INTO `estado`(`id_pais`, `id_regiao`, `nome`, `imagem_inicio`, `descricao`, `gentilico`, `populacao`, `localizacao`, `capital`, `area`, `pib`, `idh`, `text_historia`, `imagem_historia`, `text_bandeira`, `imagem_bandeira`, `text_economia`, `imagem_economia`, `text_cultura`, `imagem_cultura`, `text_clima`, `text_vegetacao`, `text_relevo`) VALUES ('$pais', '$regiao','$nome','$img_inicio','$descricao', '$gentilico', 
